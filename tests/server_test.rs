...
#[tokio::test]
async fn http_get_returns_future() -> Result {
    ...
    assert!(matches!(actual, yeti::Expression::NativeType(_)));
    let (_, actual) = yeti::evaluate_source(env, r#"(http/get "http://localhost:3001")"#)?;
    assert!(matches!(actual, yeti::Expression::Future(_)));
    Ok(())
}
