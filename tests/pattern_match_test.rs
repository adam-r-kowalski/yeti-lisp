...
async fn pattern_match_array() -> Result {
    ...
    let (_, actual) = yeti::evaluate_source(env, "((fn [[x y]] x) [1 2])").await?;
...
async fn pattern_match_map() -> Result {
    let env = yeti::core::environment();
    let (_, actual) = yeti::evaluate_source(env, "((fn [{:a a}] a) {:a 5})").await?;
...
async fn pattern_match_map_in_array() -> Result {
    let env = yeti::core::environment();
    let (_, actual) = yeti::evaluate_source(env, "((fn [[_ {:a a}]] a) [0 {:a 7}])").await?;
...
async fn pattern_match_array_with_literal_keyword() -> Result {
    let env = yeti::core::environment();
    let (_, actual) = yeti::evaluate_source(env, "((fn [[:foo y]] y) [:foo 2])").await?;
...
async fn pattern_match_array_with_literal_string() -> Result {
    let env = yeti::core::environment();
    let (_, actual) = yeti::evaluate_source(env, r#"((fn [["foo" y]] y) ["foo" 2])"#).await?;
    let expected = yeti::Expression::Integer(Integer::from(2));
    assert_eq!(actual, expected);
    Ok(())
}
...
async fn pattern_match_array_with_literal_integer() -> Result {
    let env = yeti::core::environment();
    let (_, actual) = yeti::evaluate_source(env, r#"((fn [[7 y]] y) [7 2])"#).await?;
    let expected = yeti::Expression::Integer(Integer::from(2));
    assert_eq!(actual, expected);
    Ok(())
}
...
async fn pattern_match_array_with_literal_nil() -> Result {
    let env = yeti::core::environment();
    let (_, actual) = yeti::evaluate_source(env, "((fn [[nil y]] y) [nil 2])").await?;
    let expected = yeti::Expression::Integer(Integer::from(2));
    assert_eq!(actual, expected);
    Ok(())
}
