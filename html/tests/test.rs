...
async fn evaluate_html_with_child() -> Result {
    ...
    let (_, actual) =
        compiler::evaluate_source(env, r#"(html/to-string [:ul [:li "hello"]])"#).await?;
...
async fn evaluate_html_with_two_children() -> Result {
    ...
    let (_, actual) = compiler::evaluate_source(
        env,
        r#"(html/to-string [:ul [:li "first"] [:li "second"]])"#,
    )
    .await?;
    let expected =
...
async fn evaluate_html_with_attribute() -> Result {
    ...
    let (_, actual) =
        compiler::evaluate_source(env, r#"(html/to-string [:div {:class "red"}])"#).await?;
...
async fn evaluate_html_with_attribute_and_doesnt_need_closing_tag() -> Result {
    ...
    let (_, actual) = compiler::evaluate_source(
        env,
        r#"(html/to-string [:input {:type "checkbox" :name "agree"}])"#,
    )
    .await?;
    let expected =
...
async fn evaluate_html_with_int() -> Result {
    ...
    let (_, actual) =
        compiler::evaluate_source(env, r#"(html/to-string [:ul [:li 1] [:li 2]])"#).await?;
